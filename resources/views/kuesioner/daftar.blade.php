            ...
            <div class="row align-items-center">
                ...
                <div class="col-3">
                    @if (auth()->user()->can('create kuesioner'))
                        <button type="button" class="btn btn-sm btn-primary tambah-data"
                        ...
                        <table class="table table-striped">
                            ...
                            @forelse ($kuesioner as $k)
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{route('kuesioner.kategori-kue.index',$k->id_kuesioner)}}" class="btn btn-sm btn-primary "><i class="fa fa-plus"></i> Kategori Soal</a>
                                        {{-- <a href="" class="btn btn-sm btn-info"><i class="fa fa-eye"></i></a> --}}
                                        <button type="button" data-id="{{$k->id_kuesioner}}" class="btn btn-sm btn-warning edit-data">
                                            <i class="fa fa-pencil"></i> Edit</button>
                                            <a href="#" onclick="confirmDelete(event, '{{ $k->id_kuesioner }}')" class="btn btn-sm btn-danger">
                                                <i class="fas fa-trash"></i> Hapus
                                            </a>

                                            <form id="delete-{{ $k->id_kuesioner }}" action="/kuesioner/{{ $k->id_kuesioner }}/destroy" method="POST" style="display: none;">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                                    </td>
